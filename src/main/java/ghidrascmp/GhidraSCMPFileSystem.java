...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = GhidraSCMPFileSystem.MyFileSystemFactory.class)
public class GhidraSCMPFileSystem implements GFileSystem {
	 ...
	 */
	public GhidraSCMPFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + GhidraSCMPFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<GhidraSCMPFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public GhidraSCMPFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			GhidraSCMPFileSystem fs = new GhidraSCMPFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
